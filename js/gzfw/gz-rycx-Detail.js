...
function getDetail(id, categoryid) {
	$.loadData('/api/100/500/50', '{"categoryId": "' + categoryid + '","id":"' + id + '"}', function(data) {
		...
		$msg = $('<div class="banner">' +
			...
			'<div class="info-box">' +
			'<ul>' +
			'<li><span class="title">执业机构：</span><span id="status">' + data.agencyName + '</span></li>' +
			'<li><span class="title">业务专长：</span><span id="status">' + data.businessExpertise + '</span></li>' +
			'<li><span class="title">执业证号：</span><span id="mechanism">' + data.no + '</span></li>' +
			'<li><span class="title">蒙汉双通：</span><span id="status">' + data.isMongolian + '</span></li>' +
			
			'<li class="w90"><span class="title">联系地址：</span><span id="mechanism">' + data.agencyAddress + '</span></li>' +
