$(function() {
	...
	getList(1, 1, 10, name, areaID, 'false', '', isMongolian);
					...
					jump: function(obj, first) {
						...
						$.each(list, function(index, val) {
							...
							$li = $('<li class="mediator-li" id="' + id + '" data-categoryid="' + categoryId + '">' +
								...
								'<li>法援律师：' + val.isAidLawyer + '</li>' +
								'<li>蒙汉双通：' + val.isMongolian + '</li>' +
								'<li>执业机构：' + val.agencyName + '</li>' +
							...
							$li.bind('click', function() {
								$.openSecPage('lsfw-rycx-Detail.html', {
									'id': $(this).attr('id'),
						...
						//显示默认图片
						$ul.find('.userImg').error(function() {
							$(this).attr("src", "img/mediation/mrry.jpg");
						});
...
function getAreaTown() {
	$.getArea('', function(data) {
		...
		$('.qx-list').find('li').on('click', function(e) {
			...
			getList(1, 1, 10, name, areaID, 'false', '', '');
		});
	})
}